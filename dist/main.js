...
(function () {
	...
	class BirdBox {
	        ...
	        return new Proxy(obj, {
	            get: (...args) => {
	                ...
	                if (typeof v == "function")
	                    return this.displayLoading(v.bind(target), String(prop));
	                return v;
	            }
	        });
	    }
	    ...
	    handleErrors(page) {
	        page.waitForSelector(".error-msg", { timeout: 0, visible: true }).then((elem) => { throw new Error(elem.textContent); });
	    }
	    ...
	    async mustGetState(p, options) {
	        ...
	        if (page == "UNKNOWN")
	            throw "we find ourselves in an unknown state";
	        return page;
	    }
	    async main(page) {
	        await page.show.goto('https://home.nest.com');
	        this.handleErrors(page);
	        if (await this.mustGetState(page, {}) == "login")
	            await this.logIn(page);
	        if (await this.mustGetState(page, {}) != "home")
	            throw "expected to be home by now...";
	    ...
	    async run() {
	        ...
	        await new Promise((ok, fail) => fs.access(chromeCanaryPath, fs.X_OK, (err) => err ? fail(`please install chrome canary to ${chromeCanaryPath}`) : ok()));
